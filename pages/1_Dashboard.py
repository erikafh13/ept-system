...
with c1: st.markdown('<span class="nav-logo">🎓 EPT Pro</span>', unsafe_allow_html=True)
with c3:
...
with col_start:
    ...
    if done_today:
        ...
        st.markdown("Soal baru tersedia besok setelah admin memperbarui.")
    elif total_q < 45:
        st.error(f"⚠️ Soal belum lengkap ({total_q}/45). Hubungi admin.")
        # Notifikasi admin otomatis
        if st.button("📲 Ingatkan Admin via WhatsApp"):
            ok = notify_admin_soal_belum_ada()
            if ok: st.success("Notifikasi terkirim ke admin!")
            else:  st.warning("Fitur WA belum dikonfigurasi admin.")
    else:
        st.markdown(f"""
        Soal siap: **{len(questions['listening'])} Listening** · 
        **{len(questions['structure'])} Structure** · 
        **{len(questions['reading'])} Reading**
        """)
        if st.button("▶️ Mulai Simulasi Sekarang", use_container_width=True):
            st.session_state.update({
                "test_active": True, "test_section": "listening",
                "test_idx": 0, "answers": {}, "questions_today": questions
            })
...
with col_hist:
    ...
    if df_scores.empty:
        ...
        st.download_button("⬇️ Unduh CSV", data=csv,
            file_name=f"skor_{username}.csv", mime="text/csv", use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
if not df_scores.empty and len(df_scores) > 1:
    ...
    chart = alt.Chart(melted).mark_line(point=True, strokeWidth=2).encode(
        ...
        y=alt.Y("Skor:Q"),
        color=alt.Color("Bagian:N", scale=alt.Scale(
...
with nav1:
    if st.button("📊 Lihat Analitik Saya", use_container_width=True):

"""pages/2_Admin.py — Panel kontrol admin dengan notifikasi WA."""
...
from utils.sheets import (
    ...
    add_question, delete_questions_for_date, add_user, delete_user, get_all_user_phones,
)
from utils.whatsapp import (
    notify_admin_soal_belum_ada, notify_user_reminder,
...
with c1: st.markdown('<span class="nav-logo">⚙️ Admin</span>', unsafe_allow_html=True)
with c3:
...
if total_today < 45:
    st.warning(f"⚠️ Soal hari ini baru {total_today}/45. Segera lengkapi!")
else:
    st.success(f"✅ Soal hari ini sudah lengkap ({total_today}/45)")

st.markdown("<br>", unsafe_allow_html=True)

tab_q, tab_wa, tab_scores, tab_users = st.tabs(
    ["📝 Kelola Soal", "📲 Notifikasi WhatsApp", "📊 Pantau Nilai", "👥 Kelola User"])
...
# ═══════════════════════════════════════════════════════════════════
with tab_q:
    st.markdown("### 📅 Soal per Tanggal")
    col_d, _ = st.columns([2, 3])
    with col_d:
        sel_date = st.date_input("Pilih tanggal",
            value=date.today(),
        ...
        for qtype, qlist in qd.items():
            ...
            with st.expander(f"📂 {qtype.capitalize()} ({len(qlist)} soal)"):
                for i, q in enumerate(qlist):
                    ...
                    if q.get("script"):  st.caption(f"Script: _{q['script']}_")
                    if q.get("passage"): st.caption(f"Passage: _{q['passage'][:80]}..._")
                    st.divider()
        if st.button(f"🗑️ Hapus SEMUA soal {sel_str}", type="secondary"):
    ...
    with st.form("add_q", clear_on_submit=True):
        ...
        with col_t: q_type = st.selectbox("Tipe", ["listening","structure","reading"])
        with col_n: q_no   = st.number_input("Nomor", 1, 15, 1)

        q_q = st.text_area("Pertanyaan *")
        q_script  = st.text_area("Script Audio (listening)") if q_type == "listening" else ""
        q_passage = st.text_area("Passage (reading)")        if q_type == "reading"   else ""

        ca, cb = st.columns(2)
        with ca: oa = st.text_input("Opsi A *"); oc = st.text_input("Opsi C *")
        with cb: ob = st.text_input("Opsi B *"); od = st.text_input("Opsi D *")

        correct_map = {"A (0)": 0, "B (1)": 1, "C (2)": 2, "D (3)": 3}
        correct_raw = st.radio("Kunci *", list(correct_map.keys()), horizontal=True)
...
with tab_wa:
    ...
    Pastikan sudah mengisi `token` dan `admin_number` di `secrets.toml`.
    """)

    # Status soal hari ini
    st.markdown("#### 🔍 Cek & Ingatkan Soal")
    col_cek, col_notif = st.columns(2)
    with col_cek:
        st.metric("Soal Hari Ini", f"{total_today}/45",
                  delta="Lengkap ✅" if total_today >= 45 else f"Kurang {45-total_today} soal ⚠️")
    with col_notif:
        if st.button("📨 Kirim Pengingat ke Admin (Diri Sendiri)", use_container_width=True):
    ...
    st.markdown("Kirim pesan pengingat latihan harian ke semua user yang memiliki nomor WA.")

    user_phones = get_all_user_phones()
    users_with_phone = [u for u in user_phones if u.get("phone")]

    if not users_with_phone:
        st.warning("Belum ada user dengan nomor WA. Tambahkan nomor di tab Kelola User.")
    else:
        st.markdown(f"**{len(users_with_phone)} user** siap menerima notifikasi:")
        st.dataframe(pd.DataFrame(users_with_phone)[["name","phone"]].rename(
            columns={"name":"Nama","phone":"Nomor WA"}), hide_index=True)

        if st.button(f"📲 Kirim ke {len(users_with_phone)} User Sekarang", type="primary"):
            success = fail = 0
            prog = st.progress(0)
            for i, u in enumerate(users_with_phone):
                ok = notify_user_reminder(u["phone"], u["name"])
                if ok: success += 1
                else:  fail += 1
                prog.progress((i+1)/len(users_with_phone))
            st.success(f"✅ Berhasil: {success} | ❌ Gagal: {fail}")

    st.markdown("---")
    st.markdown("#### 📋 Kirim Ringkasan Harian ke Admin")
    df_all = get_all_scores()
    today_scores = df_all[df_all["date"].dt.date == date.today()] if not df_all.empty else pd.DataFrame()

    if today_scores.empty:
        st.info("Belum ada peserta yang mengerjakan tes hari ini.")
    else:
        participants = len(today_scores)
        avg_score    = today_scores["total"].mean()
        ...
        if st.button("📨 Kirim Ringkasan ke Admin via WA", type="primary"):
            ...
            st.success("Terkirim!") if ok else st.error("Gagal. Cek konfigurasi WA.")

    st.markdown("---")
    st.markdown("#### ⚙️ Konfigurasi WhatsApp")
    st.code("""
# Tambahkan ke .streamlit/secrets.toml:

[whatsapp]
token = "TOKEN_DARI_FONNTE.COM"
admin_number = "628123456789"    # nomor WA admin
    """, language="toml")
    st.markdown("[📖 Cara mendapatkan token Fonnte →](https://fonnte.com/docs)")
...
with tab_scores:
    ...
    if df_all.empty:
        ...
        with col_f1: filter_date = st.date_input("Filter tanggal", max_d, min_d, max_d)
        with col_f2:
        ...
        if filter_u != "Semua": filtered = filtered[filtered["username"] == filter_u]

        st.dataframe(filtered[["name","date","listening","structure","reading","total","accuracy","timestamp"]].rename(
            columns={"name":"Nama","date":"Tanggal","listening":"LST","structure":"STR",
                     "reading":"RDG","total":"Total","accuracy":"Akurasi","timestamp":"Waktu"}
        ), use_container_width=True, hide_index=True)

        # Leaderboard
        st.markdown("### 🏆 Leaderboard Hari Ini")
        today_s = df_all[df_all["date"].dt.date == date.today()].sort_values("total", ascending=False).reset_index(drop=True)
        if today_s.empty:
            ...
            st.dataframe(today_s[["Rank","name","total","accuracy"]].rename(
                columns={"name":"Nama","total":"Skor","accuracy":"Akurasi"}
            ), use_container_width=True, hide_index=True)

        st.download_button("⬇️ Export Semua Nilai",
            data=df_all.to_csv(index=False).encode("utf-8"),
            file_name="semua_nilai_ept.csv", mime="text/csv")
...
with tab_users:
    ...
    if users:
        udf = pd.DataFrame([{"Username":u,"Nama":d["name"],"Role":d["role"],"No WA":d.get("phone","")}
    ...
    with st.form("add_user", clear_on_submit=True):
        ...
        if st.form_submit_button("💾 Tambah User", type="primary", use_container_width=True):
            if not all([nu, np_, nn]):
                st.error("Semua field wajib!")
            elif nu in users:
                st.error("Username sudah ada!")
            else:
                add_user(nu, np_, nn, nr, nph)
                st.success(f"✅ User '{nu}' ditambahkan!"); st.rerun()

    st.markdown("---")
    st.markdown("### 🗑️ Hapus User")
    if users:
        du = st.selectbox("Pilih user", list(users.keys()))
        if st.button("🗑️ Hapus", type="secondary"):
            delete_user(du)
            st.success(f"'{du}' dihapus."); st.rerun()
